...
The server has told us what is allowed in our kit.
This function loads what is needed.
==================
*/
void CG_SetupWeapons(void)
{
  char* kit = strdup(CG_ConfigString(CS_KIT));
  const char* tok = strtok(kit, ",");
  while (tok != NULL) {
    Com_Printf("Setting up %s\n", tok);
    trap_LoadWeapon(tok);
    tok = strtok(NULL, ",");
  }
}
...
void CG_RegisterWeapon(int weaponNum)
{
  ...
  // TODO(justinvh): We need a way to gauge what type of weapon
  // we are dealing with.

  memset(weaponInfo, 0, sizeof(*weaponInfo));
  weaponInfo->registered = qtrue;

  hat::Weapon_attrs const * weapon = NULL;
  if (!trap_GetWeaponAttributes(weaponNum, &weapon)) {
    assert(false);
  }
...
(!item->classname)
  ...
  */
  ...
  weaponInfo->weaponModel = trap_R_RegisterModel(weapon->model, qtrue);

  // try to load .md5mesh model if the .md3 could not be found
  if(!weaponInfo->weaponModel) {
    Com_Error(0, "Weapon model could not be loaded.");
  }
  ...
  // HACK(justinvh): Our models don't have viewModels yet.
  weaponInfo->viewModel = weaponInfo->weaponModel;

  const char* animation_mapping[MAX_WEAPON_STATES] = {
    "idle",
    "draw",
    "lower",
    "fire",
  ...
  hat::Animation_list::const_iterator acit;
  hat::Animation_sound_list::const_iterator scit;
  for (acit = weapon->animations.begin();
    acit != weapon->animations.end();
    ++acit)
  {
    const char* type = acit->type;
    for (int i = 0; i < MAX_WEAPON_STATES; i++) {
      if (strcmp(type, animation_mapping[i]) != 0)
        continue;

      if (!CG_RegisterWeaponAnimation(&weaponInfo->viewModel_animations[i], acit->animation, qtrue, qfalse, qfalse)) {
        CG_Error("Could not find '%s'\n", acit->animation);
      }

      for (scit = acit->sounds.begin();
  ...
  //weaponInfo->empty_sound = trap_S_RegisterSound(weapon.attributes.empty_sound.c_str());
  //memcpy(&weaponInfo->attributes, &weapon, sizeof(weaponInfo->attributes));
