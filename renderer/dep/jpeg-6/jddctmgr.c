...
#define CONST_BITS 14
				...
				SHIFT_TEMPS for(i = 0; i < DCTSIZE2; i++)
				{
				...
				for(row = 0; row < DCTSIZE; row++)
				{
					for(col = 0; col < DCTSIZE; col++)
					{
						fmtbl[i] = (FLOAT_MULT_TYPE)
							((double)qtbl->quantval[jpeg_zigzag_order[i]] * aanscalefactor[row] * aanscalefactor[col]);
						i++;
					}
				}
			}
...
GLOBAL void jinit_inverse_dct(j_decompress_ptr cinfo)
{
	...
	idct = (my_idct_ptr) (*cinfo->mem->alloc_small) ((j_common_ptr) cinfo, JPOOL_IMAGE, SIZEOF(my_idct_controller));
	...
	for(ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components; ci++, compptr++)
	{
		...
		compptr->dct_table = (*cinfo->mem->alloc_small) ((j_common_ptr) cinfo, JPOOL_IMAGE, SIZEOF(multiplier_table));
