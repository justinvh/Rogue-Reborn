...
h2v1_merged_upsample(j_decompress_ptr cinfo, JSAMPIMAGE input_buf, JDIMENSION in_row_group_ctr, JSAMPARRAY output_buf)
{
	...
	SHIFT_TEMPS inptr0 = input_buf[0][in_row_group_ctr];
...
h2v2_merged_upsample(j_decompress_ptr cinfo, JSAMPIMAGE input_buf, JDIMENSION in_row_group_ctr, JSAMPARRAY output_buf)
{
	...
	SHIFT_TEMPS inptr00 = input_buf[0][in_row_group_ctr * 2];
...
GLOBAL void jinit_merged_upsampler(j_decompress_ptr cinfo)
{
	...
	if(cinfo->max_v_samp_factor == 2)
	{
		...
		upsample->spare_row = (JSAMPROW)
			(*cinfo->mem->alloc_large) ((j_common_ptr) cinfo, JPOOL_IMAGE, (size_t) (upsample->out_row_width * SIZEOF(JSAMPLE)));
	}
	else
	{
