...
JPP((j_decompress_ptr cinfo, JSAMPARRAY output_buf, JDIMENSION * out_row_ctr, JDIMENSION out_rows_avail));
 ...
 */
{
	...
	jmain->xbuffer[0] = (JSAMPIMAGE)
		(*cinfo->mem->alloc_small) ((j_common_ptr) cinfo, JPOOL_IMAGE, cinfo->num_components * 2 * SIZEOF(JSAMPARRAY));
...
process_data_simple_main(j_decompress_ptr cinfo, JSAMPARRAY output_buf, JDIMENSION * out_row_ctr, JDIMENSION out_rows_avail)
{
	...
	(*cinfo->post->post_process_data) (cinfo, jmain->buffer,
									   &jmain->rowgroup_ctr, rowgroups_avail, output_buf, out_row_ctr, out_rows_avail);
...
process_data_context_main(j_decompress_ptr cinfo, JSAMPARRAY output_buf, JDIMENSION * out_row_ctr, JDIMENSION out_rows_avail)
{
	...
	if(!jmain->buffer_full)
	{
		if(!(*cinfo->coef->decompress_data) (cinfo, jmain->xbuffer[jmain->whichptr]))
			return;				/* suspension forced, can do nothing more */
		jmain->buffer_full = TRUE;	/* OK, we have an iMCU row to work with */
	...
	switch (jmain->context_state)
	{
		case CTX_POSTPONED_ROW:
		 ...
		 /*FALLTHROUGH*/ case CTX_PREPARE_FOR_IMCU:
		 ...
		 /*FALLTHROUGH*/ case CTX_PROCESS_IMCU:
...
process_data_crank_post(j_decompress_ptr cinfo, JSAMPARRAY output_buf, JDIMENSION * out_row_ctr, JDIMENSION out_rows_avail)
{
	(*cinfo->post->post_process_data) (cinfo, (JSAMPIMAGE) NULL,
									   (JDIMENSION *) NULL, (JDIMENSION) 0, output_buf, out_row_ctr, out_rows_avail);
}
	...
	for(ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components; ci++, compptr++)
	{
		...
		jmain->buffer[ci] = (*cinfo->mem->alloc_sarray)
			((j_common_ptr) cinfo, JPOOL_IMAGE,
			 compptr->width_in_blocks * compptr->DCT_scaled_size, (JDIMENSION) (rgroup * ngroups));
	}
}
