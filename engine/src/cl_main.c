...
#include <hat/engine/sys_loadlib.h>
...
We have to reload the CGame because the renderer
doesn't know what graphics to reload

The UI is sent a prepare state so it knows that the video subsystem
is being restarted.
=================
*/
void CL_Vid_Restart_f(void)
{
    ...
    // We have to trick the system
    cls.uiStarted = qtrue;

    // startup all the client stuff
    CL_StartHunkUsers(qfalse);

    // Force a video restart in the UI
    CL_VidUIRestart();
        ...
        if(!(cl_allowDownload->integer & DLF_NO_REDIRECT))
        {
            if(clc.sv_allowDownload & DLF_NO_REDIRECT)
            {
                Com_Printf("WARNING: server does not "
                           "allow download redirection " "(sv_allowDownload is %d)\n", clc.sv_allowDownload);
            }
        ...
        if(!useCURL)
        {
            if((cl_allowDownload->integer & DLF_NO_UDP))
            {
                Com_Error(ERR_DROP, "UDP Downloads are "
                          "disabled on your client. " "(cl_allowDownload is %d)", cl_allowDownload->integer);
                return;
            }
            else
            {
...
void CL_InitRef(void)
{
    ...
    ri.FS_ListFilteredFiles = FS_ListFilteredFiles;
