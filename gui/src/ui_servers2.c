...
static void ArenaServers_UpdateMenu(void)
{
	...
	if(g_arenaservers.numqueriedservers > 0)
	{
		...
		{
			// show progress
			Com_sprintf(g_arenaservers.status.string, MAX_STATUSLENGTH,
						"%d of %d Arena Servers.", g_arenaservers.currentping, g_arenaservers.numqueriedservers);
	...
	for(i = 0, j = 0; i < count; i++, servernodeptr++)
	{
		...
		Com_sprintf(buff, MAX_LISTBOXWIDTH,
					"%-20.20s %-12.12s %2d/%2d %-8.8s %3s %s%3d",
...
static void ArenaServers_StartRefresh(void)
{
	...
	*g_arenaservers.numservers = 0;
	...
	else if(g_servertype >= UIAS_GLOBAL1 && g_servertype <= UIAS_GLOBAL5)
	{
