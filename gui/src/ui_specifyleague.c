	...
	switch (id)
	{
		...
		case ID_SPECIFYLEAGUENAME:
			if((event == QM_LOSTFOCUS) &&
			   (Q_strncmp(playername, s_specifyleague.rankname.field.buffer, strlen(s_specifyleague.rankname.field.buffer)) != 0))
			{
...
void SpecifyLeague_MenuInit(void)
{
	...
	Q_strncpyz(s_specifyleague.rankname.field.buffer,
			   UI_Cvar_VariableString("name"), sizeof(s_specifyleague.rankname.field.buffer));
